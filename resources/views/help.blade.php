  ...
  ──────────────           ───────────────                 ────────────────
        ...
        <section id="concepts" class="help-card help-section">
            ...
            <table class="table help">
                ...
                <tr><th>Timekeeper device</th><td>DMPI's own name for a door — its register of clocks, quite separate from your hardware. Staff assignments are filed against these, not against serial numbers. Linking a reader to one is what tells ADMS who belongs on that reader; leave it unlinked and ADMS never manages that reader's user list at all (though punches still arrive and still reach payroll — those don't depend on the link).</td></tr>
                <tr><th>The command queue</th><td>ADMS never reaches out to a clock; the clocks call in. So enrolling or removing someone means <strong>posting an instruction to that clock's mailbox</strong>, which it collects on its next check-in. An unplugged reader can hold weeks of them.</td></tr>
                <tr><th>Waiting vs unconfirmed</th><td>The two halves of that mailbox, and they are never added together. <strong>Waiting</strong> has not left this server and <em>can still be cancelled</em>. <strong>Unconfirmed</strong> was handed over and the clock never reported the result — nothing here can recall it. Unconfirmed rows are kept indefinitely on purpose: they are the only record that we told a machine something and never heard back.</td></tr>
                </tbody>
            </table>
        </section>
        ...
        <section id="screens" class="help-card help-section">
            ...
            <table class="table help">
                ...
                <tr><th>Devices</th><td>Each device's online status, name/location, IN/OUT direction, and which DMPI device it's linked to. Set direction + link here, and use <strong>Sync enrollments</strong> to push users to a device. The <strong>People</strong> column shows two numbers side by side — ADMS's intended list for the clock, and what payroll <em>assigns</em> to it — because they disagree whenever a sync is owed or somebody can't be enrolled. Alongside them, <span class="badge bg-warning text-dark">N waiting</span> (still in the mailbox, still cancellable) and <span class="badge bg-secondary">N unconfirmed</span> (handed over without a result; it may need to be re-sent). Every one of those is a link into the screens below.</td></tr>
                <tr><th>Devices → People</th><td>Enrollment evidence for one physical clock, person by person: name, PIN, company, CHAPA, card, payroll id, and punch count. <strong>ADMS intended list</strong> is the server's plan, not a device inventory. <strong>Punch received here</strong> is the strongest proof a PIN works on that reader. Current commands are split into <strong>waiting for clock</strong> and <strong>sent — unconfirmed</strong>, with an obvious link to the queue.</td></tr>
                <tr><th>Devices → Timekeeper devices</th><td>Payroll's device register, from payroll's side — reached from <strong>See who's on each</strong>. Every DMPI device shows how many people it assigns, how many are eligible, and which physical clocks are linked. On the detail page, <strong>Sync this person</strong> queues only that eligible employee to the chosen clock. A blocked row opens a step-by-step guide: download employees for a missing record, or resolve the owner under <strong>PIN conflicts</strong>. If no clock is linked, the row points back to Devices to link one first. This page works before a reader is linked, so you can inspect a payroll device without risking an empty assignment list wiping a clock.</td></tr>
                <tr><th>Devices → Queue</th><td>What this server still owes one clock, reached from the waiting/unconfirmed badges. Counts per state, then each instruction in plain words (<em>Add or update</em> / <em>Remove from clock</em>) with the person named. Filter by state or by what it does. <strong>This is the only place a queued instruction can be called back</strong> — tick rows and <strong>Cancel picked</strong>, or <strong>Cancel all N waiting</strong>. Rows already handed to the device get no tick-box, because nothing here can undo them.</td></tr>
            ...
            <table class="table help">
                ...
                <tr>
                    ...
                    <td>Re-sends every currently assigned user and queues needed removals for that physical reader. Use it to repair a handed-over command the clock never confirmed or applied. Commands leave in small batches as the device checks in, so a large roster can take several polls to finish.</td>
                </tr>
                <tr>
                    <td><span class="badge bg-outline-primary border text-primary">See who's on each</span></td>
                    <td>Devices (DMPI panel)</td>
                    <td>Opens payroll's device register so you can look <em>inside</em> a timekeeper device — who it assigns, and who of those could actually be enrolled — <strong>before</strong> committing a reader to it. Reads only data already downloaded; it never calls DMPI.</td>
                </tr>
                <tr>
                    <td><span class="badge bg-danger">Cancel all N waiting</span> <span class="badge bg-light text-danger border">Cancel picked</span></td>
                    <td>Devices → Queue</td>
                    <td>Takes instructions back out of a clock's mailbox so it never sees them. <strong>Nothing already on the machine is changed.</strong> Only <em>waiting</em> instructions can go — anything handed over is the device's business now. Cancelling an <em>add</em> also clears our record that the person was sent, so the next enrollment sync queues them again properly rather than silently skipping them for good; the confirmation message tells you when that has happened.</td>
                </tr>
                <tr>
        ...
        <section id="behaviors" class="help-card help-section">
            ...
            <table class="table help">
                ...
                <tr><th>Enrollment is not only the button</th><td><strong>Four</strong> things run it, and the button is the one people rarely use: the 15-minute job, <strong>Download assignments</strong> (immediately after downloading), a full sync, and <strong>Sync enrollments</strong> on one device. So a clock starts being managed the moment it has a link and the scheduler is alive — nobody has to press anything. Worth knowing before linking a reader.</td></tr>
                <tr><th>Linking is the dangerous move, not unlinking</th><td>Setting a reader to <em>— not linked —</em> is the safe parking state: the reconcile skips it entirely, nobody is added or removed, and punches carry on as normal. <strong>Linking</strong> is what changes the machine — point a reader at a timekeeper device and everyone on it who isn't assigned there is queued for removal. Re-pointing a <em>populated</em> reader at a different device does the same. Check the device's people first.</td></tr>
                <tr><th>Unlinking doesn't empty the mailbox</th><td>Changing or clearing the timekeeper device only changes what the <em>next</em> comparison decides. Instructions already posted stay posted and the clock still collects them. To take them back, use the queue screen.</td></tr>
                <tr><th>Enrollment can't restore fingerprints</th><td>ADMS sends a person's PIN, name, and card — it has never held a fingerprint or face template and cannot send one. So re-adding somebody after a removal gets their card working again, but any finger enrolled at the machine has to be re-enrolled at the machine.</td></tr>
                <tr><th>A clock may never confirm</th><td>Instructions are marked <em>handed over</em> when collected and <em>done</em> only when the clock reports back. Some never report — the row then sits as <strong>unconfirmed</strong> indefinitely. That is not a backlog and nothing is waiting; see the queue screen for the split.</td></tr>
        ...
        <section id="activity" class="help-card help-section">
            ...
            <table class="table help">
                ...
                <tr><td><span class="badge bg-light text-dark border badge-evt">device.queue-cancelled</span></td><td>Someone called back queued instructions for a clock. Logged as a <span class="badge bg-warning text-dark">warning</span> on purpose — it is a deliberate intervention, and the message records how many went, how many were already beyond recall, and how many people will be re-queued as a result.</td></tr>
        ...
        <section id="troubleshooting" class="help-card help-section">
            ...
            <table class="table help">
                ...
                <tr>
                    <td><strong>Scheduler</strong> says <em>"Running, but no job has ever run"</em></td>
                    <td>The process is alive and every job is dying before it starts — almost always a <strong>permissions</strong> problem. Each job takes a small lock file under <code>storage/framework/cache</code> so it can't overlap itself; if something was ever run as <strong>root</strong> in the container it left files there the app user can't open, and that one error stops all five jobs. Punches stop reaching payroll while the page can only report the symptom. Confirm with <code>tail storage/logs/scheduler.log</code> — you'll see <em>"Permission denied"</em> repeating. Fix: <code>sail exec -u root laravel.test chown -R sail storage bootstrap/cache</code>, then the next minute's run works on its own. <strong>Prevention:</strong> always <code>sail artisan &lt;thing&gt;</code>, never <code>sail exec … php artisan &lt;thing&gt;</code> — the second runs as root. The update script now repairs this automatically on every run.</td>
                </tr>
                <tr>
                    <td>A clock shows <span class="badge bg-secondary">N unconfirmed</span> that never clears</td>
                    <td>Expected, and not a backlog. Those instructions were collected by the clock, which then never reported the result — so ADMS keeps them as the only evidence it was told and never heard back. Nothing is waiting and there is nothing to cancel; check the queue screen, where <em>waiting</em> will read <strong>0</strong>. If a clock <em>never</em> confirms anything, that's worth investigating on the hardware — but it doesn't block any sync.</td>
                </tr>
                <tr>
                    <td>A clock emptied itself / everyone was removed</td>
                    <td>It's linked to the wrong <strong>timekeeper device</strong> — most likely one payroll assigns nobody to, so the reconcile queued a removal for every user on it. Open <strong>Devices → See who's on each</strong> and check that device's people; an empty one is flagged. Clearing the link stops <em>future</em> changes but does <strong>not</strong> take back instructions already posted — for that, open the clock's <strong>Queue</strong> and cancel whatever is still waiting. Anything already handed over cannot be undone, and re-linking correctly restores cards and names but <em>not</em> fingerprints.</td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td>Open the Queue first. <strong>Waiting</strong> means the clock has not collected it yet; <strong>handed over — unconfirmed</strong> is not proof it ran. Click <strong>Sync enrollments</strong> to safely re-send the current assigned users, then confirm on the device's user list. The device may also reject the User ID or card format; a successful tap arriving with the right PIN is the final proof.</td>
                </tr>
                <tr>
...
php artisan payroll:sync-attendances</pre></li>
            ...
            <div class="alert alert-warning py-2 small">
                <strong>Run artisan as the app user, never as root.</strong> <code>sail artisan …</code> does this for you.
                A bare <code>sail exec …</code> or <code>docker compose exec …</code> runs as <strong>root</strong>, and anything
                artisan writes as root — cache files, logs — the app can no longer open. Because every scheduled job takes a
                cache lock before it starts, one such file stops all five jobs and punches quietly stop reaching payroll.
                See the <em>"Running, but no job has ever run"</em> row in section 9 for the repair.
            </div>
            <p class="text-muted small">Tip: every UI button maps to a command here — <strong>Download employees</strong> = <code>sync-roster</code>; <strong>Download devices</strong> = <code>sync-devices --only=devices</code>; <strong>Download assignments</strong> = <code>sync-devices --only=assignments</code> (which also reconciles); <code>payroll:sync-all</code> runs the lot; <strong>Sync to payroll now</strong> = <code>sync-attendances</code>; <strong>Sync enrollments</strong> = <code>reconcile-enrollments</code>; <strong>Start scheduler</strong> = <code>schedule:work</code>. Useful for testing or one-off runs; in normal operation the <strong>scheduler</strong> runs them for you (see Deployment). Two things have no command and are dashboard-only: looking inside a timekeeper device, and cancelling a clock's queued instructions.</p>
            ...
            <table class="table help">
                ...
                <tr><td><code>ADMS_DEVICE_COMMAND_BATCH_SIZE</code></td><td>Maximum enrollment commands handed to a clock per poll (default 50). Keep this bounded so a large roster is delivered across several small responses.</td></tr>
...
<pre>* * * * * cd /var/www/adms && php artisan scheduler:guard >> /dev/null 2>&1</pre>
            ...
            <h6>Updating</h6>
            <p>On a Docker install, use the script — it does the steps below in the right order and repairs a couple of things that bite otherwise:</p>
<pre>git pull && ./scripts/update.sh</pre>
            <p class="text-muted small">It hands <code>storage</code> and <code>bootstrap/cache</code> back to the app user <em>before</em> the first artisan command (a root-owned file there aborts the run otherwise, on exactly the box that needs fixing), and it starts the scheduler as the app user — replacing one running under the wrong owner. Manual equivalent:</p>
<pre>git pull
