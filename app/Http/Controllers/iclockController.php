...
class iclockController extends Controller
{
    ...
    public function receiveRecords(Request $request)
    {
        ...
        try {
            ...
            if (strtoupper((string) $request->input('table')) === 'USERINFO') {
                foreach (preg_split('/\\r\\n|\\r|\\n/', trim($request->getContent())) as $line) {
                    $fields = $this->parseUserInfo($line);
                    ...
                    DeviceUser::updateOrCreate(
                        ['device_sn' => $request->input('SN'), 'pin' => $pin],
                        [
            ...
            $direction = Device::where('no_sn', $request->input('SN'))->value('direction');
            foreach ($arr as $rey) {
                ...
                $q['log_type'] = LogType::resolve($direction, (int) $q['status1']);
                $q['created_at'] = now();
                $q['updated_at'] = now();
                // dd($q);
    ...
    private function parseUserInfo(string $line): array
    {
        $fields = [];

        foreach (preg_split('/\\t/', trim($line)) as $part) {
            $separator = strpos($part, '=');
            if ($separator === false) {
                continue;
            }

            $key = strtoupper(trim(substr($part, 0, $separator)));
            $fields[$key] = trim(substr($part, $separator + 1));
        }
